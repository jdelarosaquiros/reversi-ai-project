...
def main():
    ...
    p1thread = threading.Thread(target=player_handler, args=(game_server, 0), daemon=True)
    p2thread = threading.Thread(target=player_handler, args=(game_server, 1), daemon=True)
    p1thread.start()
    p2thread.start()
    ...
    with open("game_data.csv", "a") as file:
        ...
        writer.writerow([f"Pos_Importance: f(Piece_Total*2), Bit Player: {game.white_count}, Pruning Player: {game.black_count}"])
